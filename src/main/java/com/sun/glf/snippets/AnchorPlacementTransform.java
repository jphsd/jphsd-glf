 ...
 */
public class AnchorPlacementTransform{
  ...
  public static void main(String args[]){
    ...
    AffineTransform scaleShearNrotate = AffineTransform.getRotateInstance(Math.PI/4);
    scaleShearNrotate.shear(-0.2f, -0.2f);
    scaleShearNrotate.scale(4, 4);
    AffineTransform shearNrotate = AffineTransform.getRotateInstance(Math.PI/4);
    shearNrotate.shear(-0.2f, -0.2f);

    for(int i=0; i<n; i++){
      Position position = new Position(anchors[i], 40, 60, shearNrotate);      // Create new Position.
