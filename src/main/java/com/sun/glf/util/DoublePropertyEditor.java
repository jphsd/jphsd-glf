...
/**
 * Provides a UI Component to configure an int or Double
 * property.
 *
...
public class DoublePropertyEditor extends PropertyEditorSupport{
  ...
  public java.awt.Component getCustomEditor() {
    if(editor == null){
      ...
      class DoubleChangeListener implements DocumentListener, PropertyChangeListener{
	boolean settingValue = false;

	public void propertyChange(PropertyChangeEvent evt){
	  if(!settingValue){
	    Double val = (Double)getValue();
	    valueField.setText("" + val.doubleValue());
	  }
	}
	...
	public void onChange(){
	  try{
	    ...
	    if(s.equals(".") || s.equals(".") || s.equals("-") || s.equals("-."))
	      ...
	      Double val = (s.length()>0? Double.valueOf(s):new Double(0));
	      setValue(val);
	    }
      ...
      DoubleChangeListener listener = new DoubleChangeListener();
