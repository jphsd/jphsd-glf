...
/**
 * Provides a UI Component to configure an int or Float
 * property.
 *
...
public class FloatPropertyEditor extends PropertyEditorSupport{
  ...
  public java.awt.Component getCustomEditor() {
    if(editor == null){
      ...
      class FloatChangeListener implements DocumentListener, PropertyChangeListener{
	boolean settingValue = false;

	public void propertyChange(PropertyChangeEvent evt){
	  if(!settingValue){
	    Float val = (Float)getValue();
	    valueField.setText("" + val.floatValue());
	  }
	}
	...
	public void onChange(){
	  try{
	    ...
	    if(s == null || s.equals(".") || s.equals("-") || s.equals("-."))
	      ...
	      Float val = (s.length()>0? Float.valueOf(s):new Float(0));
	      setValue(val);
	    }
      ...
      FloatChangeListener listener = new FloatChangeListener();
